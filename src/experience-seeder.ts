...
export async function getUserDecisionProfile(
  ...
  for (const pattern of byType.values()) {
    pattern.approval_rate =
      pattern.total > 0 ? pattern.approved / pattern.total : 0;
    pattern.rejection_rate =
      pattern.total > 0 ? pattern.rejected / pattern.total : 0;
  }
  ...
  if (decisions && decisions.length > 0) {
    ...
    const actionIdToType = new Map(actions.map((a) => [a.id, a.type]));
