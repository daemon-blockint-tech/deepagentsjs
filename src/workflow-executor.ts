...
async function invokeSpecialist(
  ...
  const noDataPatterns =
    /(no results|not found|tidak ditemukan|no data|no matching|couldn't find|no objects)/i;
...
export async function executeWorkflow(
  ...
  if (routing.direct || !routing.workflow) {
    const result = await invokeSpecialist(registry.orchestrator, userMessage);
    return {
      routing,
