...
const gates: Record<ControlGateName, GateFn> = {
  ...
  is_write_request: (state) => {
    const writePatterns =
      /(update|change|ubah|create|buat|delete|hapus|set|propose|execute|rekomendasi|recommend|suggest)/i;
    if (writePatterns.test(state.userMessage)) {
      return { passed: true };
    }
    // Also check if any prior step suggested an action
    const actionSuggested = state.stepOutputs.some((o) =>
      /propose|recommend|suggest|should|action/i.test(o.content),
    );
    if (actionSuggested) {
      return { passed: true };
    }
