...
const enabled = Boolean(process.env.SUPABASE_TEST_URL && process.env.SUPABASE_TEST_ANON_KEY && process.env.SUPABASE_TEST_SERVICE_KEY);
...
describe.skipIf(!enabled)("Supabase RLS", () => {
  ...
  it("service role can read workspace_members", async () => {
    const { data, error } = await serviceClient.from("workspace_members").select("*");
